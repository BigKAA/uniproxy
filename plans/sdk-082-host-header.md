...
- **Активный подпункт**: Phase 1 завершена
- **Последнее обновление**: 2026-03-13
- **Примечание**: Phase 1 полностью завершена. SDK обновлён до v0.8.2, `go build ./...` и `go test ./...` проходят.
...
- [ ] [Phase 2: Документация и примеры](#phase-2-документация-и-примеры)
...
- [ ] **2.1 Обновить examples/config.yaml**
  - **Dependencies**: None
...
- [ ] **2.4 Обновить Helm chart values (при необходимости)**
  - **Dependencies**: None
...
- [ ] Примеры конфигурации корректны и покрывают основной use case (ingress/proxy)
- [ ] README содержит понятное описание и примеры
- [ ] `make helm-lint` проходит (если Helm-файлы изменены)
...
- [ ] **3.3 Локальное тестирование (Docker)**
  - **Dependencies**: 3.2
...
- [ ] `make check-all` проходит без ошибок
- [ ] Docker-образ собирается успешно
- [ ] Health check с `hostHeader` отправляет корректный Host header
- [ ] Health check с `grpcAuthority` отправляет корректный :authority
- [ ] Метрики генерируются корректно
- [ ] Нет ошибок в логах
