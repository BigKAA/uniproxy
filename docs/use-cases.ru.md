...
14. [Произвольные HTTP-заголовки](#14-произвольные-http-заголовки)
...
## 14. Произвольные HTTP-заголовки

### Проблема

Некоторые сторонние API используют пользовательские заголовки для аутентификации (например, `X-API-Key`, `X-Auth-Token`) вместо стандартных заголовков `Authorization`. Другие сервисы требуют не-auth заголовки: идентификаторы трассировки, согласование содержимого или специфичный `User-Agent`.
...
В конфигурационном файле (`CONFIG_FILE`) заголовки — нативный map под `auth.headers`, экранирование JSON не требуется. Пример Datadog из раздела выше в YAML-форме:
...
Заголовки не ограничены учётными данными. Частые не-auth сценарии:

```bash
# Корреляция трассировки между сервисами
-e DEPHEALTH_BACKEND_HEADERS='{"X-Request-Id":"health-check","X-Trace-Id":"uniproxy-01"}'

# Согласование содержимого / версионирование API
-e DEPHEALTH_API_HEADERS='{"Accept":"application/json","Accept-Version":"v2"}'

# Переопределение User-Agent "dephealth/<версия>" по умолчанию
-e DEPHEALTH_LEGACY_HEADERS='{"User-Agent":"my-probe/1.0"}'
```

> Заголовки считаются методом аутентификации и не комбинируются с `bearerToken`, basic auth или `metadata` в одной зависимости. Для проверок на основе заголовков используйте отдельную зависимость.
